...
contract ShadowQueueAprOracle is Governance {
    ...
    mapping(address vault => address[] strategiesOutsideQueue) public extraStrategies;

    IOracle public constant CORE_ORACLE = IOracle(0x1981AD9F44F2EA9aDd2dC4AD7D075c102C70aF92);
     ...
     */
    function aprAfterDebtChange(address _vault, int256 _debtChange) external view virtual returns (uint256 apr) {
        ...
        for (uint256 i = 0; i < strategies.length; i++) {
            ...
            for (uint256 j; j < strategiesOutsideQueue.length; ++j) {
                require(strategies[i] != strategiesOutsideQueue[j], "Duplicate strategy");
            }
        ...
        for (uint256 i = 0; i < strategiesOutsideQueue.length; i++) {
            uint256 debt = IVault(_vault).strategies(strategiesOutsideQueue[i]).current_debt;

            if (debt == 0) continue;

            // Get a performance fee if the strategy has one.
            (bool success, bytes memory fee) = strategiesOutsideQueue[i].staticcall(
                abi.encodeWithSelector(IStrategy(strategiesOutsideQueue[i]).performanceFee.selector)
            );
     ...
     */
    function setExtraStrategies(address _vault, address[] memory _shadowQueue) external virtual onlyGovernance {
        // make sure each strategy is attached to the vault
        IVault vault = IVault(_vault);
        for (uint256 i = 0; i < _shadowQueue.length; i++) {
            require(vault.strategies(_shadowQueue[i]).activation > 0, "!activated");
        }
