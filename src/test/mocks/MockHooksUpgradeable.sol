...
contract HookEvents {
    ...
    event PreWithdrawHook(uint256 assets, uint256 shares, address receiver, address owner, uint256 maxLoss);

    event PostWithdrawHook(uint256 assets, uint256 shares, address receiver, address owner, uint256 maxLoss);

    event PreTransferHook(address from, address to, uint256 amount);

    event PostTransferHook(address from, address to, uint256 amount, bool success);
}
...
contract MockHooksUpgradeable is BaseHooksUpgradeable, HookEvents {
    ...
    function _postTransferHook(address from, address to, uint256 amount, bool success) internal override {
